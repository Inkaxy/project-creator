...
import {
  ...
  useIKControlItems,
  useManageControlItems,
} from "@/hooks/useIKControls";

interface EditControlTemplateModalProps {
  open: boolean;
...
export function EditControlTemplateModal({
  open,
  onOpenChange,
  template,
}: EditControlTemplateModalProps) {
  ...
  const { data: existingItems = [] } = useIKControlItems(template?.id || null);
  const updateTemplate = useUpdateControlTemplate();
  ...
  useEffect(() => {
    if (template) {
      ...
      setIsCritical(template.is_critical);
    }
  }, [template]);

  // Populate items when existingItems changes
  useEffect(() => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Rediger kontrollpunkt</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          >
            {updateTemplate.isPending ? "Lagrer..." : "Lagre endringer"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
